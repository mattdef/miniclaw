...
async fn test_session_persistence_roundtrip() {
    ...
    // Create manager and add session with messages
    {
        ...
        let session = manager.get_or_create_session("telegram", "123456789").await.unwrap();
        
        // Add user message
        manager.add_message(&session.session_id, Message::new(
            "user".to_string(),
            "Hello".to_string(),
        )).await.unwrap();
        ...
        manager.add_message(&session.session_id, Message::new(
            "assistant".to_string(),
            "I'll list files".to_string(),
        ).with_tool_calls(tool_calls)).await.unwrap();
        
        // Add tool result
        manager.add_message(&session.session_id, Message::tool_result(
...
async fn test_session_recovery_statistics() {
    ...
    // Create sessions and persist them
    {
        ...
        for i in 0..5 {
            let session = manager.get_or_create_session("telegram", &format!("user_{}", i)).await.unwrap();
            manager.add_message(
                &session.session_id,
...
async fn test_corrupted_session_recovery() {
    ...
    tokio::fs::write(&session_file, "invalid json {{").await.unwrap();
...
async fn test_tool_calls_survive_context_builder_pipeline() {
    ...
    // Create session with tool interaction flow
    {
        ...
        manager.add_message(&session.session_id, Message::new(
            "user".to_string(),
            "What files are in /tmp?".to_string(),
        )).await.unwrap();
        ...
        // Tool result
        manager.add_message(&session.session_id, Message::tool_result(
            "Tool call_fs_1 result: file1.txt, file2.txt".to_string(),
        )).await.unwrap();
    ...
    // Load in new manager and verify ContextBuilder correctly formats for LLM
    {
        ...
        let context = context_builder.build_context(&session, &inbound).await.unwrap();
        
        // Verify conversation history is in context
        let user_msg = context.iter().find(|m| m.content.contains("What files are in /tmp?"));
        ...
        let assistant_msg = context.iter().find(|m| m.content.contains("I'll list the files"));
        assert!(assistant_msg.is_some(), "Assistant message with tool_calls should be in context");
        assert_eq!(assistant_msg.unwrap().role, LlmRole::Assistant);
        assert!(assistant_msg.unwrap().tool_calls.is_some(), "tool_calls should be preserved");
        assert_eq!(assistant_msg.unwrap().tool_calls.as_ref().unwrap().len(), 1);
        assert_eq!(assistant_msg.unwrap().tool_calls.as_ref().unwrap()[0].name, "filesystem");
        
        // Verify tool result is in context with correct role (LlmRole::Tool)
        let tool_result_msg = context.iter().find(|m| m.content.contains("Tool call_fs_1 result"));
        assert!(tool_result_msg.is_some(), "Tool result should be in context");
        assert_eq!(tool_result_msg.unwrap().role, LlmRole::Tool, "Tool result should have LlmRole::Tool");
        
        // Verify final assistant message
        let final_msg = context.iter().find(|m| m.content.contains("I found 2 files"));
        assert!(final_msg.is_some(), "Final assistant message should be in context");
...
async fn test_session_updates_visible_to_context_builder() {
    ...
    let mut session = manager.get_or_create_session("telegram", "456").await.unwrap();
    ...
    let context1 = context_builder.build_context(&session, &inbound).await.unwrap();
    
    let has_first_msg = context1.iter().any(|m| m.content.contains("First question"));
    ...
    let context2 = context_builder.build_context(&fresh_session, &inbound).await.unwrap();
    
    let has_first_answer = context2.iter().any(|m| m.content.contains("First answer"));
    assert!(has_first_answer, "Updated session should be visible to ContextBuilder on next iteration");
    
    // Verify message count matches
    let history_count = context2.iter().filter(|m| 
        matches!(m.role, LlmRole::User | LlmRole::Assistant) && 
        !m.content.contains("Second question")  // Exclude current message
    ).count();
    assert_eq!(history_count, 2, "Should have 2 history messages (user + assistant)");
}
