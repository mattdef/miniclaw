...
impl SkillsManager {
    ...
    pub async fn load_skills(&self) -> Result<(), SkillManagerError> {
        ...
        let mut entries = tokio::fs::read_dir(&self.skills_dir).await.map_err(|e| {
            SkillManagerError::FileReadFailed {
    ...
    pub async fn create_skill(
        ...
        tokio::fs::create_dir_all(&skill_dir).await.map_err(|e| {
            SkillManagerError::DirectoryCreationFailed {
    ...
    pub async fn read_skill(&self, name: &str) -> Result<String, SkillManagerError> {
        ...
        if !skills.contains_key(name) {
            return Err(SkillManagerError::SkillNotFound {
                name: name.to_string(),
            });
        }
    ...
    pub async fn delete_skill(&self, name: &str) -> Result<(), SkillManagerError> {
        ...
        tokio::fs::remove_dir_all(&skill_dir).await.map_err(|e| {
            SkillManagerError::DirectoryRemovalFailed {
                path: skill_dir.to_string_lossy().to_string(),
                source: e,
            }
        })?;
    ...
    fn validate_skill_name(name: &str) -> Result<(), SkillManagerError> {
        ...
        if name.contains("..") || name.contains('/') || name.contains('\\') {
            return Err(SkillManagerError::InvalidName {
                name: name.to_string(),
                reason:
                    "Name cannot contain path separators or '..' sequences (security violation)"
                        .to_string(),
            });
        }
        ...
        for (i, c) in name.chars().enumerate() {
            ...
            if !c.is_ascii_lowercase() && !c.is_ascii_digit() && c != '_' {
                return Err(SkillManagerError::InvalidName {
                    name: name.to_string(),
                    reason: format!(
                        "Invalid character '{}' at position {}. Only lowercase letters, numbers, and underscores allowed",
                        c, i
                    ),
                });
            }
    ...
    fn check_name_conflict(name: &str) -> Result<(), SkillManagerError> {
        if BUILT_IN_TOOLS.contains(&name) {
            return Err(SkillManagerError::BuiltInToolConflict {
                name: name.to_string(),
            });
        }
        Ok(())
    ...
    fn parse_skill_md(name: &str, content: &str) -> Result<ManagedSkill, SkillManagerError> {
        ...
        let created_at = content
            ...
            .and_then(|line| {
                ...
                line.split("**Created**:").nth(1).map(|s| s.trim())
            })
    ...
    async fn test_create_skill_builtin_conflict() {
        ...
        assert!(
            result
                .unwrap_err()
                .to_string()
                .contains("conflicts with built-in tool")
        );
    }
...
"#;
    ...
    async fn test_read_skill_file_deleted() {
        ...
        let skill_md_path = manager
            .skills_dir()
            .join("vanishing_skill")
            .join("SKILL.md");
    ...
    async fn test_read_skill_permission_denied() {
        ...
        let skill_md_path = manager
            .skills_dir()
            .join("forbidden_skill")
            .join("SKILL.md");
        let mut perms = tokio::fs::metadata(&skill_md_path)
            .await
            .unwrap()
            .permissions();
        perms.set_mode(0o000);
        tokio::fs::set_permissions(&skill_md_path, perms)
            .await
            .unwrap();
    ...
    async fn test_load_skills_preserves_created_at() {
        ...
        let loaded_skill = skills
            .iter()
            .find(|s| s.name == "timestamped_skill")
            .unwrap();
        ...
        assert!(
            loaded_skill
                .created_at
                .starts_with(&original_timestamp[..19])
        );
    }
}
